...
7. Nunca execute `git commit` (nem `git add`/`git push`) por conta própria.
   Sempre sugira a mensagem de commit e deixe o usuário decidir se e
   quando commitar.
8. Responda sempre em português, em qualquer situação.
```
...
| `index.html` | Versão com moldura de celular fixa (tema escuro padrão), conteúdo do meio cortado na altura da tela — pra gerar print/apresentação | Mudança visual pra mockup/apresentação |
| `index_light.html` | Mesma moldura do `index.html`, tema claro como padrão | Mesma coisa, versão clara |
...
**Regra:** o Claude Code nunca deve rodar `git add`, `git commit` ou
`git push` neste repositório por conta própria — apenas sugerir a
mensagem de commit. Quem decide se e quando commitar é o usuário.
